...
function getPedagogicNotes(item, state) {
  const notes = [];
  if (item.id.startsWith('diag_post')) {
    notes.push('Rotation de tête à 45° et observation de l’inversion au retour assis.');
    notes.push('Le côté testé positif correspond au côté atteint.');
  }
  if (item.id === 'diag_horiz_bowlean') {
    notes.push('Canalolithiase : latence puis nystagmus paroxystique.');
    notes.push('Cupulolithiase : pas de latence, nystagmus persistant.');
  }
  if (item.id === 'diag_horiz_supineroll') {
    notes.push('Géotropique = plus intense côté atteint.');
    notes.push('Agéotropique / cupulolithiase = plus intense côté sain.');
    if (state.subtype === 'cupulolithiase') notes.push('Rechercher un null point en limitant la rotation à 30–40° pour verticaliser la cupule.');
  }
  if (item.id === 'diag_horiz_upright') {
    notes.push('Géotropique : le nystagmus suit l’inclinaison.');
    notes.push('Agéotropique : le nystagmus prend le sens opposé.');
  }
  if (item.id.startsWith('diag_ant')) {
    notes.push('Un downbeat doit toujours faire discuter une cause centrale.');
    notes.push('La composante torsionnelle aide à la latéralisation si elle est visible.');
  }
  if (item.id === 'tx_post_semont') {
    notes.push('Première bascule : nystagmus géotropique vers l’oreille basse.');
    notes.push('Seconde bascule : nystagmus thérapeutique agéotropique vers l’oreille haute.');
  }
  if (item.id === 'tx_post_epley') {
    notes.push('Maintenir chaque position au moins 30 secondes.');
    notes.push('Le nystagmus thérapeutique garde la logique du provocateur puis s’inverse au retour.');
  }
  if (item.id.startsWith('tx_horiz_')) {
    notes.push('Manœuvre horizontale : nystagmus thérapeutique classiquement controlatéral à l’oreille affectée.');
    notes.push('Respecter les pauses de maintien et sécuriser le retour assis.');
  }
  if (item.id === 'tx_ant_yacovino') {
    notes.push('Pas besoin d’identifier le côté atteint avant la manœuvre.');
    notes.push('Le menton poitrine suit l’épuisement du nystagmus vertical inférieur.');
  }
  if (item.id === 'tx_ant_li') {
    notes.push('Alternative sans latéralisation préalable.');
    notes.push('La position ventrale est maintenue de façon prolongée.');
  }
  if (item.id === 'tx_ant_kim' || item.id === 'tx_ant_epley_inverse') {
    notes.push('Ces options supposent une latéralisation préalable au Dix-Hallpike.');
    notes.push('La tête part du côté sain.');
  }
  return notes;
}
  ...
  switch (item.id) {
    ...
    case 'diag_ant_deephead': {
      if (step.id === 'hanging') return {
        ...
        bubble:'Deep Head Hanging : nystagmus vertical inférieur, parfois pur, parfois accompagné d’une torsion faible.',
        details:[`torsion ${getAntTorsion(side, false)}`, 'latéralisation parfois impossible']
      };
      if (step.id === 'return') return {
...
function renderEye(model) {
  ...
  const summary = (() => {
    if (model.type === 'none') return 'Aucun nystagmus attendu';
    if (model.type === 'horizontal') return `Horizontal ${directionLabel(model.dir)}`;
    if (model.type === 'vertical' || model.type === 'vertical-possible') {
      return model.vertical === 'down' ? 'Vertical inférieur' : 'Vertical supérieur';
    }
    if (model.type === 'torsional-vertical') {
      const vertical = model.vertical === 'down' ? 'Vertical inférieur' : 'Vertical supérieur';
      return `${vertical} + torsion ${model.torsion}`;
    }
    return model.summary || 'Lecture du nystagmus';
  })();

  const linearArrow = (() => {
    if (model.type === 'horizontal') {
      return model.dir === 'right'
        ? `<line x1="28" y1="72" x2="136" y2="72" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`
        : `<line x1="136" y1="72" x2="28" y2="72" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`;
    }
    if (model.type === 'vertical' || model.type === 'vertical-possible' || model.type === 'torsional-vertical') {
      const dash = model.type === 'vertical-possible' ? 'stroke-dasharray="4 4"' : '';
      return model.vertical === 'down'
        ? `<line x1="164" y1="46" x2="164" y2="108" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" ${dash} marker-end="url(#arrowRedSmall)"/>`
        : `<line x1="164" y1="108" x2="164" y2="46" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" ${dash} marker-end="url(#arrowRedSmall)"/>`;
    }
    return '';
  })();
  ...
  const torsion = (() => {
    ...
    const antiHoraire = model.torsion === 'anti-horaire';

    if (antiHoraire) {
      return `<path d="M 62 103 Q 26 72 62 41" fill="none" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`;
    }

    return `<path d="M 102 41 Q 138 72 102 103" fill="none" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`;
  })();

  return `
    <svg viewBox="0 0 ${w} ${h}" width="100%" height="180" xmlns="http://www.w3.org/2000/svg" aria-label="Schéma du nystagmus">
      <defs>
        <marker id="arrowRedSmall" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
          <path d="M0,0 L6,3 L0,6 z" fill="#dc2626"></path>
        </marker>
      </defs>
      <rect x="0" y="0" width="${w}" height="${h}" rx="14" fill="#ffffff"/>
      <circle cx="${cx}" cy="${cy}" r="44" fill="#f8fbff" stroke="#9ab0c7" stroke-width="2.5"/>
      <circle cx="${cx}" cy="${cy}" r="13" fill="#1565c0"/>
      <circle cx="${cx - 5}" cy="${cy - 5}" r="3.5" fill="#fff"/>
      ${torsion}
      ${linearArrow}
      <text x="${w / 2}" y="155" text-anchor="middle" font-size="12.5" fill="#16324f" font-family="inherit">${summary}</text>
    </svg>
  `;
}
...
class DiagnosticsManoeuvresUI {
  ...
  render() {
    ...
    img.onerror = () => {
      ...
      placeholder.innerHTML = `<div><strong>Image manquante</strong><br/><br/>Nom attendu : <code>${step.image}</code><br/>Dossier : <code>${DM_IMAGE_BASE}</code></div>`;
    };
    placeholder.style.display = 'none';
    img.src = imagePath;

    document.getElementById('stepMeta').innerHTML = `
      <strong>Étape ${this.state.stepIndex + 1} / ${item.steps.length}</strong><br/>
      Fichier attendu : <span class="dm-filename">${step.image}</span>
      ${item.mirrorLeft && this.state.side === 'left' ? '<br/>Affichage miroir appliqué automatiquement pour l’oreille gauche.' : ''}
    `;
    ...
    document.getElementById('infoBubble').innerHTML = `
      <strong>Nystagmus attendu</strong><br/>
      ${model.bubble}<br/>
      ${(model.details || []).length ? '<div style="margin-top:8px"><strong>Repères :</strong> ' + model.details.join(' • ') + '</div>' : ''}
    `;

    const notes = getPedagogicNotes(item, this.state);
    document.getElementById('notesBox').innerHTML = `<ul class="dm-note-list">${notes.map(note => `<li>${note}</li>`).join('')}</ul>`;

    document.getElementById('currentFilesBox').innerHTML = `<ul class="plain-list dm-files-list">${item.steps.map(s => `<li><span class="dm-filename">${s.image}</span><br/><span class="small">${s.caption}</span></li>`).join('')}</ul>`;
  }
}
