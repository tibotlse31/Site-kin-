...
function renderEye(model) {
  ...
  const arrow = (() => {
    ...
    if (model.type === 'vertical' || model.type === 'vertical-possible') {
      ...
      return model.vertical === 'down'
        ? `<line x1="140" y1="56" x2="140" y2="152" stroke="#dc2626" stroke-width="6" stroke-linecap="round" ${dash} marker-end="url(#arrowRed)"/>`
        : `<line x1="140" y1="152" x2="140" y2="56" stroke="#dc2626" stroke-width="6" stroke-linecap="round" ${dash} marker-end="url(#arrowRed)"/>`;
    }
    return '';
  })();
  ...
  const torsion = (() => {
    ...
    return `
      <path d="M ${x1} ${y1} A ${r} ${r} 0 1 ${clockwise ? 1 : 0} ${x2} ${y2}" fill="none" stroke="#dc2626" stroke-width="5"/>
      <text x="140" y="176" text-anchor="middle" font-size="13" fill="#dc2626" font-family="inherit">torsion ${model.torsion}</text>
    `;
  })();

  const verticalCaption = model.type === 'torsional-vertical'
    ? `<text x="140" y="194" text-anchor="middle" font-size="13" fill="#dc2626" font-family="inherit">composante verticale ${model.vertical === 'up' ? 'supérieure' : 'inférieure'}</text>`
    : '';
