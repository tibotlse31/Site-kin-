...
function renderEye(model) {
  ...
  const linearArrow = (() => {
    if (model.type === 'horizontal') {
      return model.dir === 'right'
        ? `<line x1="34" y1="72" x2="130" y2="72" stroke="#dc2626" stroke-width="4" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`
        : `<line x1="130" y1="72" x2="34" y2="72" stroke="#dc2626" stroke-width="4" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`;
    }
    if (model.type === 'vertical' || model.type === 'vertical-possible' || model.type === 'torsional-vertical') {
      ...
      return model.vertical === 'down'
        ? `<line x1="162" y1="46" x2="162" y2="106" stroke="#dc2626" stroke-width="4" stroke-linecap="round" ${dash} marker-end="url(#arrowRedSmall)"/>`
        : `<line x1="162" y1="106" x2="162" y2="46" stroke="#dc2626" stroke-width="4" stroke-linecap="round" ${dash} marker-end="url(#arrowRedSmall)"/>`;
    }
    return '';
  })();
  ...
  const torsion = (() => {
    ...
    const r = 30;
    const start = clockwise ? Math.PI * 1.35 : Math.PI * -0.35;
    const end = clockwise ? Math.PI * -0.25 : Math.PI * 1.25;
    const x1 = cx + Math.cos(start) * r;
    const y1 = cy + Math.sin(start) * r;
    const x2 = cx + Math.cos(end) * r;
    const y2 = cy + Math.sin(end) * r;
    const sweep = clockwise ? 0 : 1;
    return `<path d="M ${x1} ${y1} A ${r} ${r} 0 1 ${sweep} ${x2} ${y2}" fill="none" stroke="#dc2626" stroke-width="4" stroke-linecap="round" marker-end="url(#arrowRedSmall)"/>`;
  })();
