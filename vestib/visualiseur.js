...
function renderEye(model) {
  ...
  const horizontalArrow = (() => {
    ...
    return model.dir === 'right'
      ? `<line x1="178" y1="104" x2="102" y2="104" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowLine)"/>`
      : `<line x1="102" y1="104" x2="178" y2="104" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowLine)"/>`;
  })();

  const verticalArrow = (() => {
    if (!['vertical', 'vertical-possible', 'torsional-vertical'].includes(model.type)) return '';
    const dash = model.type === 'vertical-possible' ? 'stroke-dasharray="5 5"' : '';
    return model.vertical === 'down'
      ? `<line x1="214" y1="78" x2="214" y2="132" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" ${dash} marker-end="url(#arrowLine)"/>`
      : `<line x1="214" y1="132" x2="214" y2="78" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" ${dash} marker-end="url(#arrowLine)"/>`;
  })();

  const torsion = (() => {
    if (model.type !== 'torsional-vertical') return '';
    if (model.torsion === 'horaire') {
      return `<path d="M 108 128 Q 88 104 108 80" fill="none" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowCurve)"/>`;
    }
    return `<path d="M 172 128 Q 192 104 172 80" fill="none" stroke="#dc2626" stroke-width="3.5" stroke-linecap="round" marker-end="url(#arrowCurve)"/>`;
  })();

  return `
    <svg viewBox="0 0 ${w} ${h}" width="100%" height="220" xmlns="http://www.w3.org/2000/svg" aria-label="Schéma du nystagmus">
      <defs>
        <marker id="arrowLine" markerWidth="5" markerHeight="5" refX="4.5" refY="2.5" orient="auto" markerUnits="strokeWidth">
          <path d="M0,0 L5,2.5 L0,5 z" fill="#dc2626"></path>
        </marker>
        <marker id="arrowCurve" markerWidth="4.5" markerHeight="4.5" refX="4.1" refY="2.25" orient="auto" markerUnits="strokeWidth">
          <path d="M0,0 L4.5,2.25 L0,4.5 z" fill="#dc2626"></path>
        </marker>
      </defs>
